...
<head>
    ...
    Ext.onReady(function() {
        var listData = new Ext.data.Store({
            reader: new Ext.data.JsonReader({
                fields: [
                   'id_sub_kategori_neraca','id_kategori_neraca','nama_sub_kategori_neraca'
                ],
                root: 'rows', totalProperty: 'results'
            }),
            proxy: new Ext.data.HttpProxy({
                url: BASE_URL + 'sub_kategori_neraca/get_all_data',
                method: 'POST'
            })
        });
	...
	var formAdd= new Ext.form.FormPanel({
	    url:BASE_URL + 'sub_kategori_neraca/add',
        baseCls: 'x-plain',
        labelWidth: 90,
		fileUpload:true,
        items: [
			 {
			xtype: 'textfield',
		fieldLabel: 'Id Kategori Neraca',
		id:'id_kategori_neraca1',
		anchor: '80%',
		name: 'id_kategori_neraca'
		}, 
		{
		 xtype: 'textfield',
		fieldLabel: 'Nama Sub Kategori Neraca',
		anchor: '80%',
		id:'nama_sub_kategori_neraca1',
		name: 'nama_sub_kategori_neraca'
		}
		],
		buttons: [{
            text: 'Save',
...
var window = new Ext.Window({
	...
	var formEditData= new Ext.form.FormPanel({
	    url:BASE_URL + 'sub_kategori_neraca/editData',
         fileUpload: true,
		baseCls: 'x-plain',
        labelWidth: 90,
   ...
   icon: BASE_ICONS + 'edit.png',
			...
			handler: function()
			{
				...
				if(m.length > 0)
				{				
					formEditData.getForm().load({url:BASE_URL + 'sub_kategori_neraca/getData/'+ m[0].get('id_sub_kategori_neraca'), waitMsg:'Loading'});
					windowEdit.show();			 
				}
				else
				{
                ...
                fn: function(btn) {
                    if (btn == 'yes') {
                        ...
                        for (i = 0; i<sel.length; i++) {
                            data = data + sel[i].get('id_sub_kategori_neraca') + ';';
                        }
						


                        Ext.Ajax.request({
                            url: BASE_URL + 'sub_kategori_neraca/delete',
                            method: 'POST',
       ...
       var dataTable = new Ext.grid.EditorGridPanel({
            ...
            columns: [
              new Ext.grid.RowNumberer(), cbGrid, {
			        header: 'Id Kategori Neraca',
                    dataIndex: 'id_kategori_neraca',
                    sortable: true,
                    width: 100
                }, { 
		             header: 'Nama Sub Kategori Neraca',
                    dataIndex: 'nama_sub_kategori_neraca',
                    sortable: true,
                    width: 100
                }  
            ],
    ...
    <title>sub_kategori_neraca</title>
</head>
<body>
