...
<head>
    ...
    Ext.onReady(function() {
        var listData = new Ext.data.Store({
            reader: new Ext.data.JsonReader({
                fields: [
                   'id_kategori_neraca','kategori_neraca'
                ],
                root: 'rows', totalProperty: 'results'
            }),
            proxy: new Ext.data.HttpProxy({
                url: BASE_URL + 'kategori_neraca/get_all_data',
                method: 'POST'
            })
        });
	...
	var formAdd= new Ext.form.FormPanel({
	    url:BASE_URL + 'kategori_neraca/add',
        baseCls: 'x-plain',
        labelWidth: 90,
		fileUpload:true,
        items: [
		...
		{
		 xtype: 'textfield',
		fieldLabel: 'Kategori Neraca',
		anchor: '80%',
		id:'kategori_neraca1',
		name: 'kategori_neraca'
		}
		],
		buttons: [{
            text: 'Save',
...
var window = new Ext.Window({
	...
	var formEditData= new Ext.form.FormPanel({
	    url:BASE_URL + 'kategori_neraca/editData',
         fileUpload: true,
		baseCls: 'x-plain',
        labelWidth: 90,
   ...
   icon: BASE_ICONS + 'edit.png',
			...
			handler: function()
			{
				...
				if(m.length > 0)
				{				
					formEditData.getForm().load({url:BASE_URL + 'kategori_neraca/getData/'+ m[0].get('id_kategori_neraca'), waitMsg:'Loading'});
					windowEdit.show();			 
				}
				else
				{
                ...
                fn: function(btn) {
                    if (btn == 'yes') {
                        ...
                        for (i = 0; i<sel.length; i++) {
                            data = data + sel[i].get('id_kategori_neraca') + ';';
                        }
						


                        Ext.Ajax.request({
                            url: BASE_URL + 'kategori_neraca/delete',
                            method: 'POST',
       ...
       var dataTable = new Ext.grid.EditorGridPanel({
            ...
            columns: [
              new Ext.grid.RowNumberer(), cbGrid, { 
		             header: 'Kategori Neraca',
                    dataIndex: 'kategori_neraca',
                    sortable: true,
                    width: 100
                }  
            ],
