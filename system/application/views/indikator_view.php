    ...
    Ext.onReady(function() {
        var listData = new Ext.data.Store({
            reader: new Ext.data.JsonReader({
                fields: [
                   'Id_indikator','Id_Jenis','Dari','Sampai','Warna'
                ],
                root: 'rows', totalProperty: 'results'
            }),
            proxy: new Ext.data.HttpProxy({
                url: BASE_URL + 'indikator/get_all_data',
                method: 'POST'
            })
        });
...
var window = new Ext.Window({
	...
	var formEditData= new Ext.form.FormPanel({
	    url:BASE_URL + 'indikator/editData',
         fileUpload: true,
		baseCls: 'x-plain',
        labelWidth: 90,
   ...
   icon: BASE_ICONS + 'edit.png',
			...
			handler: function()
			{
				...
				if(m.length > 0)
				{				
					formEditData.getForm().load({url:BASE_URL + 'indikator/getData/'+ m[0].get('Id_indikator'), waitMsg:'Loading'});
					windowEdit.show();			 
				}
				else
				{
                ...
                fn: function(btn) {
                    if (btn == 'yes') {
                        ...
                        for (i = 0; i<sel.length; i++) {
                            data = data + sel[i].get('Id_indikator') + ';';
                        }
