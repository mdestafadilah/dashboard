...
<head>
    ...
    Ext.onReady(function() {
        var listData = new Ext.data.Store({
            reader: new Ext.data.JsonReader({
                fields: [
                   'id_item','id_sub_level_neraca','nama_neraca','value'
                ],
                root: 'rows', totalProperty: 'results'
            }),
            proxy: new Ext.data.HttpProxy({
                url: BASE_URL + 'sub_item/get_all_data',
                method: 'POST'
            })
        });
	...
	var formAdd= new Ext.form.FormPanel({
        ...
        items: [
				 {
			xtype: 'combo',
		fieldLabel: 'Sub Level Neraca',
		id:'id_sub_level_neraca1',
		transform:'subleveladd',
         hiddenName:'id_sub_level_neraca',
         typeAhead: true,
         forceSelection: true,
		triggerAction: 'all',
		emptyText: 'Sub Level',
...
var window = new Ext.Window({
	...
	var formEditData= new Ext.form.FormPanel({
	    url:BASE_URL + 'sub_item/editData',
         fileUpload: true,
		baseCls: 'x-plain',
        labelWidth: 90,
	...
	reader: new Ext.data.JsonReader ({
        ...
        items: [
			...
			xtype: 'combo',
		fieldLabel: 'Sub Level Neraca',
		id:'id_sub_level_neraca2',
		transform:'subleveledit',
         hiddenName:'id_sub_level_neraca',
         typeAhead: true,
         forceSelection: true,
		triggerAction: 'all',
   ...
   icon: BASE_ICONS + 'edit.png',
			...
			handler: function()
			{
				...
				if(m.length > 0)
				{				
					formEditData.getForm().load({url:BASE_URL + 'sub_item/getData/'+ m[0].get('id_item'), waitMsg:'Loading'});
					windowEdit.show();			 
				}
				else
				{
       ...
       var dataTable = new Ext.grid.EditorGridPanel({
            ...
            columns: [
              new Ext.grid.RowNumberer(), cbGrid, {
			        header: 'Id Sub Level Neraca',
                    dataIndex: 'id_sub_level_neraca',
                    sortable: true,
                    width: 100
                }, {
			        header: 'Nama Neraca',
                    dataIndex: 'nama_neraca',
                    sortable: true,
                    width: 100
                }, { 
		             header: 'Value',
...
<body>
 ...
 <div id='dataTable'></div>
  ...
  <?php
       $kategori="";
       $sqlkategori=$this->db->query("select * from sub_level_neraca order by nama_sub_neraca asc");
       foreach($sqlkategori->result() as $rowkategori){
       $kategori .="<option value='".$rowkategori->id_sub_level_neraca."'>".$rowkategori->nama_sub_neraca."</option>";
       }
        ?>
        <select id="subleveladd">
        <?=$kategori?>
        </select>
        <select id="subleveledit">
        <?=$kategori?>
        </select>
