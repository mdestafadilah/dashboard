...
if($_POST['B2']=="Import"){
   ...
   for($i=$_POST['baris'];$i<=$data->sheets[0]['numRows'];$i++){
       ...
       if($nrowtotal==0){
          $kal="insert into temp2(phoneNo,phoneName,status) values('".$notarget."','".$data->sheets[0]['cells'][$i][$_POST['kolom1']]."','".$_POST['statusphone']."')";
...
if($b[$a]==0)$alias[$a]=$ex2[$b[$a]];
 ...
 $kal1="select phoneNo from phone where phoneNo='".$no[$a]."'";
	   ...
	   $nrowtotal=mysql_num_rows($nrow);
       if($nrowtotal==0){
$sql[$a]="insert into temp2(phoneName,phoneNo,status)Values('".$alias[$a]."','".$no[$a]."','".$_POST['statusph']."')";
...
function gothere() { 
 var selectList = document.getElementById("typeofdata");
  ...
  window.open("list.php?status=<?=$_GET['status']?>&&id=<? echo $_GET['id'] ?>&showlink=<? echo $_POST['B1']=='Link Data' ? 1 : ($_GET['showlink']==1 ? 1 : '') ?>&dwiz="+nilai,"_self");
}
</script>
...
function gomapping0() { 
 var selectList = document.getElementById("dbcolumn0");
  ...
  var selectOptions = selectList.options;
  var nilai = selectList[selectIndex].value;
  window.open("list.php?status=<?=$_GET['status']?>&&id=<? echo $_GET['id'] ?>&showlink=<? echo $_POST['B1']=='Link Data' ? 1 : ($_GET['showlink']==1 ? 1 : '') ?>&dwiz=<? echo $dwiz ?>&dbconnect=<? echo $dbconnect ?>&dbtables=<? echo $dbtables ?>&columns1=<? echo $columns[1]=='' ? $_GET['columns1'] : $columns[1] ?>&columns0="+nilai,"_self");
}
</script>
...
<body bgcolor="#FFFFFF">
<center><h2>Data Mapping</h2></center><p>
<form name="import" enctype="multipart/form-data" action="list.php?status=<?=$_GET['status']?>&&showlink=<? echo $_POST['B1']=='Link Data' ? 1 : ($_GET['showlink']==1 ? 1 : '') ?>#bk_preview" method="POST">
...
<tfoot></tfoot>
  ...
  <tr class="row0">
    ...
    <td width="40%" align="left">&nbsp;<a href="list.php?status=<?=$_GET['status']?>&&id=<? echo $_GET['id'] ?>&dwiz=2&dispex=1&namafileexcel=<? echo $v_namafile[$i] ?>"><? echo $v_namafile[$i] ?></a></td>
    <td width="20%" align="center"><? echo $v_filetgl[$i] ?></td>
  </tr>
  <?php }?>
</table>
<INPUT TYPE="hidden" name="jumfile" value="<? echo count($v_namafile) ?>">
<INPUT TYPE="hidden" name="stat" value="<? echo $_GET['status'] ?>">
...
<tfoot></tfoot>
  ...
  <tr class="row0">
    ...
    <td width="40%" align="left">&nbsp;<a href="list.php?status=<?=$_GET['status']?>&&id=<? echo $_GET['id'] ?>&dwiz=1&distxt=1&namafiletxt=<? echo $v_namafile[$i] ?>"><? echo $v_namafile[$i] ?></a></td>
    <td width="20%" align="center"><? echo $v_filetgl[$i] ?></td>
  </tr>
  <?php }?>
</table>
